...
pretrained_train_sys_retriever = IndexRetriever(datasets=[train_set],
                                            embedding_filenames=[
    f"{PRETRAINED_MODEL_SAVE_PATH}/mw21_train_sys_{PRETRAINED_MODEL}.npy"],
    search_index_filename=f"{PRETRAINED_MODEL_SAVE_PATH}/mw21_train__sys_{PRETRAINED_MODEL}.npy",
    sampling_method="pre_assigned",
)

# pretrained retreiver for usr
pretrained_train_usr_retriever = IndexRetriever(datasets=[train_set],
                                            embedding_filenames=[
    f"{PRETRAINED_MODEL_SAVE_PATH}/mw21_train_usr_{PRETRAINED_MODEL}.npy"],
    search_index_filename=f"{PRETRAINED_MODEL_SAVE_PATH}/mw21_train__usr_{PRETRAINED_MODEL}.npy",
    sampling_method="pre_assigned",
)
...
class MWDataset:
    ...
    def __init__(self, mw_json_fn,  just_embed_all=False):
        ...
        # separate history
        self.turn_sys_hist = [] # store context + sys utterance
        self.turn_usr_hist = [] # store context + usr utterance
        ...
        for turn in data:
            ...
            # make separate history
            sys_hist, usr_hist = input_to_separate_string(context, sys_utt, usr_utt)
