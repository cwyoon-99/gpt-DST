...
custom_prompt_with_neg_ex = """Your task is to find the changed domain-slots based on the context and the dialogue between user and system, and find the corresponding value.
The following lists are domain-slots and their possible values.
you don't have to find other changed domain-slots if they are not in the list.
...
def get_bracket_prompt(data_item, examples, given_context=None, n_examples=None):
...
def get_bracket_neg_ex_prompt(data_item, examples, given_context=None, n_examples=None):
    
    question_item = data_item

    prompt_text = f"{conversion(custom_prompt_with_neg_ex)}\n"
