...
special_token_prompt = """<|im_start|>**instructions**
...
- The following lists are domain-slots and their possible values.
- !IMPORTANT : you don't have to find other changed domain-slots if they are not in following lists.
<|im_end|>
...
def get_special_token_prompt(data_item, examples, given_context=None, n_examples=None):
    
    question_item = data_item

    prompt_text = f"{conversion(special_token_prompt)}\n\n\n"
    ...
    if max_n_examples > 0:
        for example_id, example in enumerate(examples[-max_n_examples:]):
            ...
            prompt_text += f"[system] {last_sys_utt}\n"
            prompt_text += f"[user] {example['dialog']['usr'][-1]}\n"
            prompt_text += f"Q: Based on current dialogue states ([context]), system utterance ([system]), and user utterance ([user]), what are changed (domain-slot = value)?\n"
